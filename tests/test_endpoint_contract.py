...
class EndpointLocalIntervalContractLint(unittest.TestCase):
    ...
    def test_producer_schema_fixes_independent_checker_inputs(self):
        ...
        self.assertNotIn("series", upper_rule["then"]["properties"])
        self.assertIn(
            "Psi_prime",
        ...
        derivative_upper_series = (
            derivative_items["allOf"][0]["then"]["properties"]["series"]
        )
        self.assertEqual(derivative_upper_series["minItems"], 2)
        self.assertEqual(derivative_upper_series["maxItems"], 2)
        self.assertEqual(
            {
                rule["contains"]["properties"]["function"]["const"]
                for rule in derivative_upper_series["allOf"]
            },
            {"Psi", "Psi_prime"},
        )
        endpoint_items = (
        ...
        endpoint_upper_series = (
            endpoint_items["allOf"][0]["then"]["properties"]["series"]
        )
        self.assertEqual(endpoint_upper_series["minItems"], 2)
        self.assertEqual(endpoint_upper_series["maxItems"], 2)
        self.assertEqual(
            {
                rule["contains"]["properties"]["function"]["const"]
                for rule in endpoint_upper_series["allOf"]
            },
            {"Phi", "Psi"},
        )
        self.assertNotIn(
