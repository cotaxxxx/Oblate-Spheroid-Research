...
def full():
    ...
    while queue:
        ...
        print("C1B_ATTEMPT_BEGIN", attempts, slab.coarse, slab.depth, slab.ll, slab.lr)
        ok, rec, root, tc, work, reason = attempt(slab, previous_root)
        for k,v in work.items(): totals[k] += v
        attempt_work = sum(work.values())
        print("C1B_ATTEMPT_END", attempts, "status", "ACCEPT" if ok else "REFINE_OR_ABORT",
              "reason", reason, "work", work, "total", attempt_work)
        if attempt_work > ATTEMPT_WORK_CEILING + 513*2*PRED_SCAN_PANELS:
        ...
        if ok:
            ...
            print("C1B_LAMBDA_REFINE", slab, "->", left, right)
            queue = [left, right] + queue
    slabs = [r["slab"] for r in accepted]
    union_ok = (bool(slabs) and slabs[0].ll == L_LO and slabs[-1].lr == L_HI
                and all(a.lr == b.ll for a,b in zip(slabs, slabs[1:])))
    lambda_b = min((r["slab"].ll for r in accepted if r["right_clamp"]), default=None)
    right_clamped = sum(1 for r in accepted if r["right_clamp"])
    corners = sum(r["corner_hull"] for r in accepted)
    actual = sum(totals.values())
    print("C1B_FINAL_LAMBDA_UNION", "PASS" if union_ok else "FAIL",
          "accepted", len(accepted), "attempted", attempts,
          "first", (slabs[0].ll, slabs[0].lr) if slabs else None,
          "last", (slabs[-1].ll, slabs[-1].lr) if slabs else None,
