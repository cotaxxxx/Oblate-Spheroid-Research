...
def _corner_kernel(s, t, lam):
    ...
    # A-priori hulls from rho^2(2-s^2)+lambda^2 phi^2=1.
    rho_hi = 1 / gap.lower().sqrt()
    rho = _box(arb(0), rho_hi)
    inv_lam_hi = (1 / lam).upper()
    phi = arb(0, inv_lam_hi)
    Ahat = gap * s * rho - mu * phi

    # No q-division in the corner chart. sqrt(q) is multiplicative only.
    sqrtq = q.sqrt()
...
def _ordinary_kernel(s, t, lam, degree):
    ...
    gamma_raw = lam * A / (w * sqrtq)
    gamma = _unit_hull(gamma_raw)
    u_raw = e * gap * _square(h_t) / (w2 * q)
    u = _unit_hull(_clamp_nonnegative(u_raw))

    gt = -lam * e * H / (w * q32)
    gtt = lam3 * e * (3 * d * H - gap * q) / (w * q52)

    if _contains_zero(h_t):
        if not u.upper() < 1:
        ...
        if not u.lower() > 0:
            raise ValueError("gamma_lower requires u separated from zero")
        R = gamma.acos() / u.sqrt()
        Rg = (gamma * R - 1) / u
        chart = "gamma_lower"
    ...
    for ti, (tl, tr) in enumerate(t_boxes):
        ...
        for li, (ll, lr) in enumerate(l_boxes):
            ...
            for si, (sl, sr) in enumerate(zip(s_endpoints, s_endpoints[1:])):
                ...
                s = _box(sleft, sright)
                is_corner = ti == T_SPLITS - 1 and si == 0
                if is_corner:
            ...
            passed = bool(total.upper() < 0)
            all_pass = all_pass and passed
            records.append({
                "t_box": [str(tl), str(tr)],
                "lambda_box": [str(ll), str(lr)],
                "chart_counts": chart_counts,
                "total_mid": total.mid().str(50),
                "total_rad": total.rad().str(50),
                "upper_negative": passed,
            })

    return {
        "schema": "bg-oblate-spheroid.monotone-tube.v1",
