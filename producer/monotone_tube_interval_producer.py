...
def _quantities(s,t,lam):
    ...
    q=_clamp_nonnegative(e*gap+lam2*_square(d)); w2=lam2*e*gap+_square(mu); w=w2.sqrt()
    ht=mu+lam2*d; H=(1-e)*gap+lam2*(2*e-2*delta-e*e+delta*e)
...
def _ordinary(s,t,lam,degree):
    ...
    if use_u:
        ...
        R,_=_series(u,"Psi",degree,clamped_nonnegative=True); Psip,_=_series(u,"Psi_prime",degree,clamped_nonnegative=True)
        Rg=-2*gamma*Psip; chart="u_upper"
    else:
    ...
    for ti,(tl,tr) in enumerate(tboxes):
        ...
        for li,(ll,lr) in enumerate(lboxes):
            ...
            records.append({"t_box":[str(tl),str(tr)],"lambda_box":[str(ll),str(lr)],"chart_counts":counts,
                "total_mid":total.mid().str(50),"total_rad":total.rad().str(50),"upper_negative":passed})
    return {"schema":"bg-oblate-spheroid.monotone-tube.v1","status":"PROTOTYPE_NOT_AUDITED_NOT_BINDING",
        "contract":{"t_domain":["63/64","1"],"lambda_domain":["5/8","33/50"],"t_boxes":8,"lambda_boxes":8,
            "s_panels":panels,"series_degree":degree,"bits":bits,"required_sign":"NEG","sole_gate":"every parameter-box total.upper() < 0"},
