...
def _density(s,t,lam):
    ...
    def upper():
        ...
        return -s*mu*Phi + 2*lam*R*Ahat*_pow(rho,3)*H/w
    def lower():
        if not u.lower()>0: raise ValueError("lower invalid")
        alpha=gamma.acos(); R=alpha/u.sqrt()
        return -s*mu*alpha*alpha + 2*lam*R*Ahat*_pow(rho,3)*H/w
    th=_point(USTAR)
